...
3. Дополнил информацию о книге:
       русифицировал жанры;
       добавил номер серии;
       сократил отбражение пути к файлу;
       размер книги теперь отображается корректно;
       добавил отображение языка книги;
4. Добавил онлайн читалку, чтоб полистать книгу, предже чем её скачивать.
...
- для коррекного отображения жанров и аннотаций придётся снести коллекцию.
  **python3 manage.py sopds_util clear [--verbose]**
  Потом надо отсканировать её заново. Не пугайресь - это быстро. 
...
3) Добавил аннотации книг. Опыт показал, что данные в inp файлах более упорядочены.
   Но аннотации есть лишь в fb2 файлах. Так что параллельно сканируем зазипованные fb2.
4) Увеличил размер обложек - нифига ж не видно.
5) Убрал длиннющий путь имени файла. Вместо "имяфайла.inpx/имяфайла.inp/имяфайла.zip/имякниги.fb2" теперь отображается "имяфайла.zip/имякниги.fb2"
6) Правильное отображение размера fb2 файла. Оказывается в django для этого есть встроенная функция.
7) Перевёл placeholder поисковой строки на русский.
8) Самое главное - просмотр книг онлайн.
   ...
   Пока не получилось. Всё равно огромная благодарность [@bookpauk](https://github.com/bookpauk) за либераму.
   Я не читаю книги в браузере, но часто необходимо пролистать пару страниц
   "по диагонали" чтоб понять, та ли это книга.


Спасибо [@iAHTOH](https://github.com/iAHTOH) за наводку на sopds и за тёмную тему.
Спасибо [@zveronline](https://github.com/zveronline) за Docker.
Ну и нижайший поклон [@mitshel](https://github.com/mitshel)
